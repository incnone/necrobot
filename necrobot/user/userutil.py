...
def get_user(
    ...
    if user_id is not None and user_id in user_library:
        return user_library[user_id]
    ...
    if not raw_db_data:
        if not register:
            ...
            user_library[user.user_id] = user
            return user
            ...
            if discord_member is not None:
                ...
                user_library[user.user.user_id] = user
                return user
        else:
...
def commit_all_checked_out_users():
    for user in user_library.values():
        user.commit()


def _get_user_from_db_row(user_row):
    user_id = int(user_row[8])
    if user_id in user_library:
        return user_library[user_id]
