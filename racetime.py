def from_str(time_str):    #time_str in the form [m]:ss.hh; returns the time in total hundredths of a second, or -1 on failure
    args = time_str.split(':')
    if len(args) == 2:
        args_2 = args[1].split('.')
        if len(args_2) == 2 and len(args_2[0]) == 2 and len(args_2[1]) == 2:
            try:
                t_min = int(args[0])
                t_sec = int(args_2[0])
                t_hun = int(args_2[1])
